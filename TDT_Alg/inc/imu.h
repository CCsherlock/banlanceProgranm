...
public:
    ...
    bool judgeMove();
    bool nowMoveState,lastMoveState;
		int moveJudCnt;
    struct SixCalibrationParam
    {
        u8 nowCaliAxis = 0;
        u8 totalCnt = 0;
        int powOfacc;
        int accSixSum[6];
        int gyroSixSum[6];
        float accSixOffset[6];
        float gyroSixOffset[6];
        float fixFactor[3] = {1, 1, 1};
        int nowCauculateNum;
        bool isMoving = 1;
        int16_t movingJudge = 1000;
        bool waitNextAxis = 1;
        bool calibrationOk = 1;
        float g = 4095.875f; // 2048.0f;
    };
    struct SixCalibrationParam sixCail;
    float sixCaliOffset[3] = {0, 0, 0};
    float sixCaliFector[3] = {1, 1, 1};
    float gyroCaliOffset[3] = {0, 0, 0};
