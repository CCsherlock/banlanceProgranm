...
#define ROBOT_WHEEL_RADIUS 210

#define OUTER_WHEEL_RADIO 5
#else
...
#define ENUM_START(x)                                   \
	const unsigned char x##_ENUM_START_LINE = __LINE__; \
	enum x
...
#define USE_OPERATOR_AUTO_INC(x)       \
	inline x operator++(x &rs, int)    \
	{                                  \
		x oldEnum = rs;                \
		if ((int)rs < ENUM_NUM(x) - 1) \
			rs = (x)(rs + 1);          \
		else                           \
			rs = (x)0;                 \
		return oldEnum;                \
