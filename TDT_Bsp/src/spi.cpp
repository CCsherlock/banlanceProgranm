...
namespace SPI{
	...
	CsPin defaultCsPin[3] ={{GPIOA,GPIO_Pin_4} , {GPIOB,GPIO_Pin_0} , {0,0}};
	#else
...
void Spi::init(void)
{
	...
	if(spix == SPI1 && initFlag[SPI::spi1] == false)
	{
		...
		gpioInitStructure.GPIO_Pin =  GPIO_Pin_3|GPIO_Pin_4;//PA5~7复用功能输出
		GPIO_Init(GPIOB, &gpioInitStructure);//初始化	
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource3,GPIO_AF_SPI1); //PA5复用为 SPI1  CLK//PB3
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource4,GPIO_AF_SPI1); //PA6复用为 SPI1  MISO//PB4
		
		gpioInitStructure.GPIO_Pin =  GPIO_Pin_7;//PA5~7复用功能输出
	...
	else if(spix == SPI2 && initFlag[SPI::spi2] == false)
	{
		...
		RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE);//使能SPI2,3时钟
		gpioInitStructure.GPIO_Pin =  GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;//PA5~7复用功能输出	
		GPIO_Init(GPIOB, &gpioInitStructure);//初始化	
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource13,GPIO_AF_SPI2); //PA5复用为 SPI1  CLK//PB3
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource14,GPIO_AF_SPI2); //PA6复用为 SPI1  MISO//PB4
		GPIO_PinAFConfig(GPIOB,GPIO_PinSource15,GPIO_AF_SPI2); //PA7复用为 SPI1  MOSI//PB5
...
{
  u8 rx_data;
	for(int i=0;i<0xfffffff && SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET;i++);//等待发送区空  
	SPI_I2S_SendData(SPI1, txdata); //通过外设SPIx发送一个bit  数据
	for(int i=0;i<0xfffffff && SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) == RESET;i++);//等待接收完一个bit  
	rx_data = SPI_I2S_ReceiveData(SPI1); //返回通过SPIx最近接收的数据	
  return rx_data;
}
void BMI088_write_single_reg(u8 reg, u8 data)
{
    BMI088_read_write_byte(reg);
    BMI088_read_write_byte(data);
}
void BMI088_read_single_reg(u8 reg, u8 *return_data)
{
    BMI088_read_write_byte(reg | 0x80);
    *return_data = BMI088_read_write_byte(0x55);
}
void BMI088_read_muli_reg(u8 reg, u8 *buf, u8 len)
{
    BMI088_read_write_byte(reg | 0x80);
