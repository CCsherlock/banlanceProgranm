...
#else
    chassisTorque[LEFT] = (roboLqr->resultValue[roboLqr->OUT_LEFT_MOTOR] / OUTER_WHEEL_RADIO) * chassisSetPossitive * chassisResultKp * chasTorKpByMotion * chasTorKpBySlide;   // 底盘输出力矩乘以系数
    chassisTorque[RIGHT] = (roboLqr->resultValue[roboLqr->OUT_RIGHT_MOTOR] / OUTER_WHEEL_RADIO) * chassisSetPossitive * chassisResultKp * chasTorKpByMotion * chasTorKpBySlide; // 底盘输出力矩乘以系数
    chassisTorque[LEFT] = LIMIT(chassisTorque[LEFT], -MAX_CHASSIS_T, MAX_CHASSIS_T);
    chassisTorque[RIGHT] = LIMIT(chassisTorque[RIGHT], -MAX_CHASSIS_T, MAX_CHASSIS_T);   
